...
namespace GameEngine.Data.Model
{
    public class SpriteAnimated : Sprite
    {
        ...
        public SpriteAnimated(Texture Text, int FrameWidth, int FrameHeight, int FramesPerSecond, int FirstFrame = 0, int LastFrame = 0, bool IsAnimated = false, bool IsLooped = true)
            : base(Text)
        {
            _fps = FramesPerSecond;
            _interval = (int)(1000.0 / (double)_fps);
        ...
        /// </summary>
        public void Update(double elapdsedTime)
        {
            _clock += elapdsedTime;
            if (_isAnimated & _clock >= _interval)
            {
                this.TextureRect = this.GetFramePosition(_currentFrame);
                //this.Origin = new Vector2f(this.TextureRect.Left, this.TextureRect.Top);
